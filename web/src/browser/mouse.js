...
export function MouseAdapter(bus, screen_container)
{
    ...
    this.init = function()
    {
        ...
        window.addEventListener("touchstart", touch_start_handler, { passive: false });
        window.addEventListener("touchend", touch_end_handler, { passive: false });
        window.addEventListener("touchmove", mousemove_handler, { passive: false });
    ...
    function may_handle(e)
    {
        ...
        // Touch events: always handle when mouse is enabled (target check is unreliable on touch)
    ...
    function is_ui_element(target)
    {
        while(target && target !== document.body)
        {
            if(target.classList && (
                ...
                target.tagName === "A" ||
                target.closest("#runtime_options") ||
                target.closest("#runtime_infos") ||
                target.closest("#boot_options")
            ))
            {
                return true;
            }
    ...
    function touch_start_handler(e)
    {
        ...
        if(is_ui_element(e.target)) return;
        ...
        if(touches && touches.length)
        {
            ...
            // Left button down
            left_down = true;
            mouse.bus.send("mouse-click", [true, false, false]);
        }
    }
    ...
    function mousemove_handler(e)
    {
        ...
        // Touch move: use relative delta (PS2 mouse only supports relative movement)
        var touches = e["changedTouches"];
        if(touches)
        {
            // Don't intercept touches on UI elements
            ...
            {
                var touch = touches[touches.length - 1];
                var delta_x = touch.clientX - last_x;
                var delta_y = touch.clientY - last_y;
                last_x = touch.clientX;
                last_y = touch.clientY;
        ...
        {
            const rect = screen_container.getBoundingClientRect();
            // Use touch coordinates for touch events, mouse coordinates otherwise
