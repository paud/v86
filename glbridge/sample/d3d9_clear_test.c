// Minimal Direct3D 9 -> v86 WebGPU bridge smoke test.
//
// This program deliberately imports d3d9.dll only. It exercises exactly the
// M1 scope of glbridge/d3d9proxy/d3d9_proxy.c: Direct3DCreate9, CreateDevice,
// Clear, BeginScene/EndScene, Present. It is the D3D9 equivalent of
// ../sample/d3d8_clear_test.c and is meant to be the very first thing run
// against the DLL -- before any real game -- to prove the guest DLL does not
// crash and that a batch actually reaches the host executor.
//
...
static const char g_window_class[] = "V86GLD3D9ClearTest";
...
static void trace_hresult(const char *stage, HRESULT hr)
{
    ...
    wsprintfA(line, "[d3d9-smoke] %s -> 0x%08lX\r\n",
...
static void set_result_title(HWND hwnd, const char *stage, HRESULT hr)
{
    ...
    wsprintfA(title, "D3D9 smoke: %s (0x%08lX)",
            stage, (unsigned long)hr);
...
static HRESULT init_d3d9(HWND hwnd)
{
    D3DDISPLAYMODE mode;
    D3DPRESENT_PARAMETERS present_parameters;
    HRESULT hr;

    trace_text("Direct3DCreate9 begin");
    g_d3d = Direct3DCreate9(D3D_SDK_VERSION);
    if (!g_d3d)
    {
        trace_text("Direct3DCreate9 returned NULL");
        g_failed_stage = "Direct3DCreate9 returned NULL";
        return E_FAIL;
    }
    trace_text("Direct3DCreate9 returned an interface");
...
static int run_test(HINSTANCE instance, int show_command)
{
    ...
    hwnd = CreateWindowA(g_window_class, "D3D9 smoke: starting",
    ...
    if (FAILED(hr))
    {
        ...
        MessageBoxA(hwnd,
                "The D3D9 smoke test failed. Check the window title, guest "
                "debug output, and v86gl logs for the HRESULT and last call.",
                "D3D9 smoke test", MB_OK | MB_ICONERROR);
    }
