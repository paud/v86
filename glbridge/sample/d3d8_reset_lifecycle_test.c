...
static HRESULT verify_and_preload_extended_textures(const char *stage)
{
    ...
    if (dxt_desc.Width != 8 || dxt_desc.Height != 8
            || dxt_desc.Format != D3DFMT_DXT5)
...
static HRESULT run_reset_test(void)
{
    ...
    if (FAILED(hr)) return hr;
    hr = verify_and_preload_extended_textures("pre-Reset preload DXT5 mip chain");
    if (FAILED(hr)) return hr;
    ...
    hr = verify_and_preload_extended_textures("post-Reset preload DXT5 mip chain");
    if (FAILED(hr)) return hr;
    ...
    begin_stage("release DEFAULT-pool VB before second Reset");
    hr = IDirect3DDevice8_SetStreamSource(g_device, 0, NULL, 0);
    if (FAILED(hr)) return fail_stage("unbind default VB before second Reset", hr);
    refs = IDirect3DVertexBuffer8_Release(g_default_vb);
    g_default_vb = NULL;
    if (refs != 0) return fail_stage("default VB leaked before second Reset", E_FAIL);
    fill_pp(&pp, OLD_WIDTH, OLD_HEIGHT, TRUE);
    begin_stage("second Reset backbuffer 720x480 -> 480x360");
    hr = IDirect3DDevice8_Reset(g_device, &pp);
    if (FAILED(hr)) return fail_stage("second Reset 480x360", hr);
    hr = verify_and_preload_extended_textures(
            "post-second-Reset preload DXT5 mip chain");
    if (FAILED(hr)) return hr;
    hr = create_default_vb(OLD_WIDTH, OLD_HEIGHT);
    if (FAILED(hr)) return hr;
    hr = render_present("post-second-Reset Present 480x360");
    if (FAILED(hr)) return hr;
    SetWindowTextA(g_window,
            "D3D8 Reset: PASS - cycles | repeated resize | 2D/DXT managed kept | default rebuilt");
    return D3D_OK;
}
