...
//
// Two transport paths:
//   1. Windows 95/98/Me: driverless — VxDCall allocates contiguous physical
//      memory, IN/OUT instructions write the PCI I/O BAR directly (IOPL=3).
//   2. Windows 2000/XP:  v86gl.sys kernel driver via \\.\v86gl + IOCTL.
//
// Build (XP, default):
...
//
// Build (Win9x compatible, subsystem 4.0):
//   i686-w64-mingw32-gcc -shared -Os -s -nostdlib -Wl,--subsystem,windows:4.00 -Wl,-e,_DllMain@12 -o opengl32.dll opengl32_proxy.c opengl32.def -Wl,--kill-at -luser32 -lgdi32 -lkernel32
...
/* v86gl_win9x.h included later, after transport globals are defined */
...
/* Driverless transport (Win9x): physical address of g_dma_buffer for DMA.
 * When g_use_driverless is TRUE, submit via direct I/O instead of IOCTL. */
static uint32_t g_dma_phys_addr = 0;
static BOOL     g_use_driverless = FALSE;

/* Win9x driverless transport functions - must be included after the
 * transport globals above, since the static functions reference them. */
#include "v86gl_win9x.h"
...
static void close_v86gl(void) {
    ...
    if (g_use_driverless) {
        if (g_v86gl_ready) {
            v86gl_trace("unmap driverless transport frame=%lu pendingCommands=%lu pendingBytes=%lu",
                        (unsigned long)g_frame_id,
                        (unsigned long)g_dma_command_count,
                        (unsigned long)g_dma_size);
        }
        win9x_close_transport();
        g_v86gl_ready = FALSE;
        g_use_driverless = FALSE;
        g_dma_buffer = NULL;
        g_dma_capacity = 0;
        g_dma_phys_addr = 0;
        g_dma_size = 0;
...
static int open_v86gl(void) {
    ...
    /* Try driverless transport first (Win9x direct I/O, no kernel driver). */
    if (!g_use_driverless && win9x_open_transport()) {
        g_use_driverless = TRUE;
        g_v86gl_ready = TRUE;
        v86gl_trace("transport: driverless (Win9x direct I/O) phys=%08lx capacity=%lu",
                    (unsigned long)g_dma_phys_addr,
                    (unsigned long)g_dma_capacity);
        return 1;
    }

    /* Fall back to \\.\v86gl kernel driver (Win2k/XP). */
...
static int emit_pci_batch(BOOL force_present) {
    ...
    /* Driverless path: direct I/O to PCI BAR (Win9x). */
    if (g_use_driverless) {
        ULONG submit_flags = force_present ? V86GL_SUBMIT_FORCE_PRESENT : 0;
        if (!win9x_submit(desc_len, submit_flags)) {
            v86gl_error("driverless SUBMIT failed frame=%lu commands=%lu bytes=%lu",
                        (unsigned long)desc->frame_id,
                        (unsigned long)desc->command_count,
                        (unsigned long)desc->command_bytes);
            close_v86gl();
        ...
        v86gl_trace("submit <- driverless accepted frame=%lu",
                    (unsigned long)desc->frame_id);
        reset_pci_stream();
        return 1;
    }

    /* Driver path: IOCTL to v86gl.sys (Win2k/XP). */
